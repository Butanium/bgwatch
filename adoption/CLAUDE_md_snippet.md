...
`bgwatch` wakes you only for: failure lines (Traceback, Error, Killed, OOM, … — `--fail RE` replaces the default, `--fail-also RE` extends it, `--ignore RE` drops lines), your `--match RE` progress marker, a heartbeat backing off from 1 to 10 min (`--every S` fixed, `--every-min/--every-max S` to reshape the backoff), a stall warning scaled to the job's own output cadence (`--stall S` to fix it), plus `--probe CMD` on every wake when the real progress signal is outside the log (an API-side batch count, a queue depth), and an `[EXIT]` line with the tail when the job ends — then it exits, so `persistent: true` never leaks. Job end is detected because the job holds the watched file open (no PID or `--pgrep` needed when the job writes that file); `--pid N` / `--pgrep PATTERN` / `--slurm JOBID` only for a job that doesn't. `bgwatch --print-defaults` shows the default patterns; `bgwatch --once FILE|ID` is the one-line status check to use instead of `cat`-ing the file.
